"""
RoBERTa Vectorization Strategy with Sliding Window
"""
import time
import numpy as np
import torch
...
class RoBERTaStrategy(VectorizerStrategy):
    """
    RoBERTa com janela deslizante (sliding window) otimizada:
      - chunk_size = 512 (inclui special tokens)
      - stride = 510 (sem sobreposição)
      - agg = 'mean'
    Evita recompressão desnecessária operando diretamente com token IDs.
    """
    def __init__(self, model_name="roberta-base", batch_size: int = 16):
        ...
        log.info(f"RoBERTaStrategy initialized: model={self.model_name}, batch_size={self.batch_size}, "
    ...
    def tokenize_and_lemmatize(self, data: DataFrame, language: str = "english"):
        log.info("Skipping manual tokenization — handled by RoBERTa tokenizer.")
        return data

    def _chunk_text_to_token_ids(self, text: str):
        """Divide texto em listas de token_ids, sem redecodificação."""
        if text is None:
            return [[self.tokenizer.bos_token_id, self.tokenizer.eos_token_id]]
        ...
        while i < total_tokens:
            ...
            window = [self.tokenizer.bos_token_id] + window + [self.tokenizer.eos_token_id]
    ...
    def _chunk_texts(self, texts: list[str]):
        ...
        log.info(f"Built {len(all_chunks)} chunks from {len(texts)} documents (RoBERTa).")
    ...
    def _encode_token_chunks(self, all_chunks: list[list[int]]) -> np.ndarray:
        ...
        log.info(f"RoBERTa encoded {n} chunks -> embeddings shape {result.shape}")
        return result
    ...
    def vectorize(self, data: DataFrame, params: dict = None):
        if data.shape[1] != 1:
            raise ValueError("RoBERTaStrategy expects exactly one text column.")
    ...
    def transform(self, fitted_vectorizer, data: DataFrame):
        if data.shape[1] != 1:
            raise ValueError("RoBERTaStrategy expects exactly one text column.")
        log.info("RoBERTa transform using fitted vectorizer.")
        column = data.columns[0]
        texts = data[column].astype(str).tolist()
        X_vectorized = self._encode_with_sliding_window(texts)
